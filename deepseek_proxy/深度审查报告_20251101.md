...
| 编号 | 区域 | 问题描述 | 影响 | 严重度 |
|------|------|----------|------|--------|
| B1 | JWT/登录 | 登录限制基于 `LoginLimiter` 的 TTL，与注释“1分钟”不一致：TTL 实际来自 `token_ttl_seconds` 配置 (可>60秒)，可能导致预期之外的复用窗口。 | 用户可长时间复用旧 Token（若 TTL 很大），影响安全与并发控制策略一致性 | 中 |
...
| B11 | 用户创建 | 未校验用户名合法字符，可能创建带路径或特殊字符的文件名，导致文件系统问题或安全风险。 | 文件系统出错/潜在注入 | 中 |
